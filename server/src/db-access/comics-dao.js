...
async function findComicById(comicId) {
    const db = getDB();
    const foundComic = db.collection("Manga").findOne({ _id: new ObjectId(comicId) });
    return foundComic;
}

async function findComicByTitle(title) {
    const db = getDB();
    const allComics = db.collection("Manga").find({ title: title }).sort({ title: -1 }).sort({ comicNumberFromSeries: -1 }).toArray();
    return allComics;
}

async function findComicByAuthor(author) {
    const db = getDB();
    const allComics = db.collection("Manga").find({ author: author }).sort({ title: -1 }).toArray();
    return allComics;
}

async function findComicByGenre(genre) {
    const db = getDB();
    const allComics = db.collection("Manga").find({ genre: genre }).sort({ title: -1 }).toArray();
    return allComics;
}

async function findOneshot() {
    const db = getDB();
    const allComics = db.collection("Manga").find({ series: false }).sort({ title: -1 }).toArray();
    return allComics;
}

async function findOpenSeriesComics() {
    const db = getDB();
    const allComics = db.collection("Manga").find({ seriesfinished: false }).sort({ title: -1 }).toArray();
    return allComics;
}

async function findUnfinishedComics() {
    const db = getDB();
    const allComics = db.collection("Manga").find({ finished: false }).sort({ addedAt: 1 }).toArray();
    return allComics;
}

async function findComicsInPossession() {
    const db = getDB();
    const allComics = db.collection("Manga").find({ stillInPossession: true }).sort({ title: -1 }).toArray();
    return allComics;
}

async function findComicNOTInPossession() {
    const db = getDB();
    const allComics = db.collection("Manga").find({ stillInPossession: false }).sort({ title: -1 }).toArray();
    return allComics;
}
...
async function insertNewComic(comic) {
    const db = getDB();
    const insertionResult = db.collection("Manga").insertOne(comic);
